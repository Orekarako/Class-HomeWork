// -----------------------------------  1A ------------------------------
let workers = [{ name: "", age: 0, salary: 0 }], hoursOfWorking = [{ regular: 0, bonus: 0, education: 0 }], sumWorkerSalary = [0];
...
class makeSalary {
    ...
    constructor(regular: number, bonus: number, whichWorker: number) {
...
function randomNewEmployee(number) {
    ...
    let randomName = "", randomAge = 0, randomSalary = 0;
    /// new Name random

    for (let i = 1; i <= Math.round(Math.random()*(4-2)+2); i++) {
        let randomLetter = String.fromCharCode(Math.round((Math.random() * (121 - 97)) + 97))
...
function giveHours() {
    ...
    for (let i = 0; i < workers.length; i++) {
        let regular = Math.round(Math.random() * (80) + 80),
            bonus = Math.round(Math.random() * (60 - 30) + 30),
...
console.log(workers); console.log(hoursOfWorking);
